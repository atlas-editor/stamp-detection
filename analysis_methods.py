...
import cv2
import numpy as np
from stamp_feature_analysis import stamp_like_features
...
def _find_contours(image_container: ImageContainer, threshold: cv2.Mat,
    ...
    for c in contours:
        ...
        stamp_like = stamp_like_features(stamp_candidate, document_width=image_container.width, document_height=image_container.height)
...
def find_colored_objects_ycrcb(image_container: ImageContainer) -> List[List[int]]:
    ...
    for i in range(2):
        ...
        thresh = create_binary_image(c)

        # find contours in the threshold
        object_coordinates += _find_contours(image_container, thresh)
...
def find_colored_objects_hsv(image_container: ImageContainer, color_lower_bound: int, color_upper_bound: int) -> \
    ...
    return _find_contours(image_container, thresh)
...
def cascade_classifier(image_container: ImageContainer, width_lower_bound: int = 680, width_upper_bound: int = 1000,
                       height_lower_bound: int = 450, height_upper_bound: int = 550) -> List[List[int]]:
    """
