import numpy as np
from stamp_feature_analysis import StampFeatureSettings, density_in_bound, dimension_in_bound, stamp_like_features, \
    wh_ratio_in_bound
...
def test_dimension_in_bound() -> None:
    ...
    # these define the lower bound for width as 50, for height 10 and the upper bounds as 250 and 500 for width and
    # height respectively
    dimension_lower_bound_factors = (10, 100)
    dimensions_upper_bound_factors = (2, 2)

    assert dimension_in_bound(stamp_width, stamp_height, img_width, img_height, dimension_lower_bound_factors,
                              dimensions_upper_bound_factors) == True
...
def test_dimension_out_bound() -> None:
    ...
    # the bounds from these values are [50, 250] for width and [100, 500] for height
    dimension_lower_bound_factors = (10, 10)
    dimensions_upper_bound_factors = (2, 2)

    assert dimension_in_bound(stamp_width, stamp_height, img_width, img_height, dimension_lower_bound_factors,
                              dimensions_upper_bound_factors) == False
...
def test_stamp_like_features() -> None:
    ...
    filled = 255 * np.ones([10, 20, 3], dtype=np.uint8)
    stamp = np.vstack((unfilled, filled))
    # the features of this artificial image are: density=20%, width=50, height=20, wh_ratio=2.5; clearly in bound for
    # parameters
...
def test_stamp_unlike_features() -> None:
    ...
    filled = 255 * np.ones([10, 20, 3], dtype=np.uint8)
    stamp = np.vstack((unfilled, filled))
    # the features are same as in `test_stamp_like_features`, but now we defined the upper bound for density as 11%
