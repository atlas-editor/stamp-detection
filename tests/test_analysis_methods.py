...
import cv2
import numpy as np
from analysis_methods import AnalysisSettings, ImageContainer, find_br_objects_hsv, find_circles, find_colored_objects_ycrcb
...
def _prepare_test(color = List[int]) -> Tuple[ImageContainer, List[int], AnalysisSettings]:
    ...
    """
    parameters = AnalysisSettings(hough_min_circle_radius=3, hough_max_circle_radius=100, stamp_feature_settings=stamp_feature_analysis.StampFeatureSettings(density_upper_bound=1))
